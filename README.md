...
3주차 공통 UI 위에 인증 상태 복원, 보호 경로, 데모 로그인과 가입, 내 프로필 조회·수정, 공개 프로필 조회를 구현했습니다. 4주차 API는 배포되지 않았으므로 MSW로 응답을 재현합니다. 프로젝트 등록·지원·관리, 팀원 검색, 알림 등은 아직 준비 화면입니다.
...
데모 계정은 `20260001` / `demo1234`입니다. 신규 가입 흐름은 `20260002` / `demo1234`로 시작합니다. 실제 세종대 포털 비밀번호는 입력하지 마세요. 프로필 이름을 `저장실패`로 저장하면 서버 오류, `네트워크오류`로 저장하면 네트워크 오류 상태를 확인할 수 있습니다.
